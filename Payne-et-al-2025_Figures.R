...
xipho_sp_fry_size_plot <-
  ...
  geom_jitter(alpha=0.6,width=0.15, cex=0.9) +
  stat_summary(fun.data = mean_se, width=0.5, size=0.8, geom = "crossbar", fill=NA) +
...
embryo_dev_profiles <- ggplot(combined_stage_partial_residuals, aes(x=stage, y=visregRes, fill=species, color=species)) +
  geom_jitter(aes(col=species),alpha=0.6,width=0.2, cex=1.3) +
...
embryo_dev_profiles <- ggplot(combined_stage_partial_residuals, aes(x=stage, y=visregRes, fill=species, color=species)) +
  geom_jitter(aes(col=species),alpha=0.6,width=0.15, cex=1.3) +
  scale_color_manual(values=color_list,breaks=c('Xmalinche', 'Xbirchmanni'),labels=c('Xmal', 'Xbir')) +
  stat_summary(fun.data = mean_se, width=0.7, size=0.8, geom = "crossbar", fill=NA) +
  theme_minimal() +
  theme(
...
stage0_size_violin <- ggplot(stage0_partial_residuals, aes(x=species, y=visregRes, col=species, fill=species)) + theme_bw() +
  ...
  geom_jitter(alpha=0.6,width=0.15, cex=1.3) +
...
lab_cross_embryo_size <- ggplot(lab_cross_partial_residuals, aes(x=species, y=visregRes, fill=species, color=species)) +
  scale_color_manual(values=color_list) +
  scale_fill_manual(values=color_list) +
  geom_jitter(alpha=0.6,width=0.1, cex=1.3) +
...
combined_subset_summed <- combined_subset_summed %>% filter(!is.na(mother_std_length))
brood_size_full <- lm(brood_size ~ species+season+mother_std_length,combined_subset_summed)
brood_size_reduced <- lm(brood_size ~ species+mother_std_length,combined_subset_summed)
anova(brood_size_full,brood_size_reduced)
# keep mother_std_length (p=1.355e-13), dropped season (0.11)
brood_size_fit <- lm(brood_size ~ species+mother_std_length, data = combined_subset_summed)
emmeans(brood_size_fit, list(pairwise ~ species), adjust = "tukey")
# 1                       estimate   SE df t.ratio p.value
# Xbirchmanni - Xmalinche     3.17 1.48 94   2.139  0.0350
